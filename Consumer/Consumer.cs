    ...
    public static async Task Run(int number, TaskCompletionSource taskCompletionSource)
    {
            ...
            using (var connection = connectionFactory.CreateConnection())
            {
                using (var channel = connection.CreateModel())
                {
                    ...
                    // Note we are listening messages from the queue still.
                    // But our queue are gonna be getting messages from exchange.
                    var queue = channel.QueueDeclare();
                    
                    // now we declare exchange
                    var fanOutExchangeConfig = new FanOutExchangeConfig();
                    
                    // we still are not going to consume from exchange directly.
                    // We bind our queue that belongs to our consumer to the exchange that we decalred.
                    channel.QueueBind(
